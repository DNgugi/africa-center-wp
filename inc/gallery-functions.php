 ...
 */
function wpac_register_gallery_block_styles()
{
    ...
    if (function_exists('register_block_style')) {
        ...
        register_block_style(
            ...
            array(
                'name'         => 'wpac-slideshow',
                'label'        => __('Slideshow', 'wpac'),
                'style_handle' => 'wpac-gallery-styles',
            )
        );
    }
}
// Use after_setup_theme hook with priority 11 to ensure it runs after theme setup
add_action('after_setup_theme', 'wpac_register_gallery_block_styles', 11);

/**
 * Add custom CSS for gallery styles.
 */
function wpac_gallery_styles()
{
    ...
    wp_add_inline_style('wpac-gallery-styles', $css);
...
add_action('wp_enqueue_scripts', 'wpac_gallery_styles');
add_action('enqueue_block_editor_assets', 'wpac_gallery_styles');

/**
 * Register custom block patterns for galleries.
 */
function wpac_register_gallery_block_patterns()
{
    ...
    register_block_pattern_category(
        'wpac-galleries',
        array('label' => __('WPAC Galleries', 'wpac'))
    );
    ...
    register_block_pattern(
        'wpac/gallery-slideshow',
        array(
            'title'       => __('Gallery Slideshow', 'wpac'),
            'description' => __('A slideshow layout for image galleries', 'wpac'),
            'categories'  => array('wpac-galleries'),
            'content'     => '<!-- wp:gallery {"linkTo":"media","className":"is-style-wpac-slideshow"} --><figure class="wp-block-gallery has-nested-images columns-1 is-cropped is-style-wpac-slideshow"><!-- wp:image {"sizeSlug":"large","linkDestination":"media"} --><figure class="wp-block-image size-large"><img src="' . get_template_directory_uri() . '/images/placeholder-1.jpg" alt="Gallery Image 1"/><figcaption class="wp-element-caption">Image caption</figcaption></figure><!-- /wp:image --><!-- wp:image {"sizeSlug":"large","linkDestination":"media"} --><figure class="wp-block-image size-large"><img src="' . get_template_directory_uri() . '/images/placeholder-2.jpg" alt="Gallery Image 2"/></figure><!-- /wp:image --><!-- wp:image {"sizeSlug":"large","linkDestination":"media"} --><figure class="wp-block-image size-large"><img src="' . get_template_directory_uri() . '/images/placeholder-3.jpg" alt="Gallery Image 3"/></figure><!-- /wp:image --></figure><!-- /wp:gallery -->',
        )
    );
}
